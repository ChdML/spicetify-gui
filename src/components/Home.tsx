...
function Home() {
  ...
  return (
    <>
      ...
      <CmdButton
        modalTitle="Upgrade Command"
        notification={"Successfully Upgraded Spicetify!"}
      ...
      <CmdButton
        onClick={() => {
          ...
          if (backupRef.current !== null)
            backupRef.current.innerText = formatTime(
              new Date(Number(localStorage.getItem("last_backup"))),
              true,
              false
            );
        }}
...
const CmdButton = ({
  ...
  const [process_id, setProcess_id] = useState(undefined);
  ...
  win.cmd_done = () => {
    ...
    setDone(true);
    // @ts-ignore
    window.kill_proc = undefined;
  };
  win.set_proc_id = (proc_id: number) => {
    setProcess_id(process_id);
    // @ts-ignore
    window.kill_proc = () => {
      // @ts-ignore

      __TAURI_INVOKE__("kill_proc", {
        pid: Number(proc_id),
      });
      // @ts-ignore
      window.kill_proc = undefined;
  ...
  return (
    <>
      ...
      <Modal
        title={`${modalTitle} / ${done ? "Done" : "Running"}`}
        open={running}
        setOpen={(bool) => {
          if (done) setRunning(bool);
          else {
            toast.loading("Killing process...");
            win.kill_proc();
          }
        }}
      >
        <div className="cmd_window"></div>
      </Modal>
    </>
  );
};
